...
impl<T: Neg<Output=T> + Sub<Output=T> + Add<Output=T> + Mul<Output=T> + Copy + PartialOrd> Line<T> {
    pub fn from_offset(v: Vector<T>, c: T) -> Self {
        Self {
            v, c
        }
    }
    ...
    pub fn from_points(p: &Point<T>, q: &Point<T>) -> Self {
        Self {
            v: q - p,
            c: (q - p).cross(&p)
        }
    ...
    pub fn side(&self, p: &Point<T>) -> T {
        self.v.cross(&p) - self.c
    }

    pub fn cmp_proj(&self, p: &Point<T>, q: &Point<T>) -> bool {
